...
message() {
  ...
  echo "    psql -U $POSTGRES_USER -W $POSTGRES_PASSWORD -h<host>               "
  echo "                                                                        "
...
if [ $MODE == 'master' ]; then
  ...
  psql $POSTGRES_DB -U $POSTGRES_USER -c "CREATE EXTENSION IF NOT EXISTS btree_gist;"
  psql $POSTGRES_DB -U $POSTGRES_USER -c "CREATE EXTENSION IF NOT EXISTS bdr;"
  psql $POSTGRES_DB -U $POSTGRES_USER -c "SELECT bdr.bdr_group_create(
  ...
  psql $POSTGRES_DB -U $POSTGRES_USER -c "CREATE EXTENSION IF NOT EXISTS btree_gist;"
  psql $POSTGRES_DB -U $POSTGRES_USER -c "CREATE EXTENSION IF NOT EXISTS bdr;"
  psql $POSTGRES_DB -U $POSTGRES_USER -c "SELECT bdr.bdr_group_join(
    local_node_name := '${HOSTNAME}',
